...
int32_t ProcessCommand(uint8_t *pu8Buffer, uint32_t u32BufferLen)
{
    ...
    switch(gCmd.u8Cmd)
    {
        ...
        case HID_CMD_LED_ON:
        {
            printf("HID_CMD_LED_ON\n"); 
            PWM_Set_Status_Led(STATUS_LED_BLUE);
            break;
        }
        case HID_CMD_LED_OFF:
        {
            printf("HID_CMD_LED_OFF\n"); 
            PWM_Set_Status_Led(STATUS_LED_OFF);
            break;
        }
        case HID_CMD_LED_FLASH:
        {
            printf("HID_CMD_LED_FLASH\n"); 
            PWM_Set_Status_Led(STATUS_LED_BLUE_FLASH);
            break;
        }
        case HID_CMD_LED_SLOW_BLINK:
        {
            printf("HID_CMD_LED_BLINK\n"); 
            PWM_Set_Status_Led(STATUS_LED_BLUE_BLINK);
            break;
        }
        case HID_CMD_LED_BREATH:
        {
            printf("HID_CMD_LED_BREATH\n"); 
            PWM_Set_Status_Led(STATUS_LED_BLUE_BREATH);
            break;
        }
        case HID_CMD_LED_PWM_DUTY:
        {
            printf("HID_CMD_LED_PWM_DUTY %d\n", gCmd.u32Arg1 ); 
            PWM_Set_Status_Led_Level(gCmd.u32Arg1);
            break;
        }
        default:
            return -1;
